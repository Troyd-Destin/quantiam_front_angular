...
export class TimesheetRtoViewComponent implements OnInit {
    ...
    createTimeRequest()
    {
      ...
      const params  = {date: this.requestTime.date,  end_hour: this.requestTime.end_hour, hours: this.requestTime.hours, 
        requestID: this.requestTime.requestID, start_hour: this.requestTime.start_hour, type: this.requestTime.type, };
    ...
    notifySupervisor()
    {
        ...
        this.rto.requested_time.forEach((data)=>{
          let date = "<tr><td align='center'>"+ moment(data.date).format('dddd')+"</td><td align='center'>" +  moment(data.date).format('MMMM Do, YYYY') + "</td><td align='center'>"+data.hours+" hrs</td><td align='center'>"+data.type+"</td><td height=0 width=0></td></tr>";
						total_hours = total_hours + data.hours;
						amount_of_days = amount_of_days + 1;
            dates.push(date);
        });

        let table = '<table border="1" cellspacing="2" cellpadding="2" width="550" >' +
                     "<th> Day of Week </th> <th> Day  </th><th>Amount</th><th>Type</th><th>Comma</th>" +
                 "<tbody>"
                         + dates +
            "</tbody></table>";

          let body = "<p><a href='"+document.location.href+"'> View Time-off Request</a></p><b> Total Time  </b>: "+total_hours+" Hours <br> <b> Total Days </b> : "+amount_of_days+"<br><br>" + table + "<br><br><p><i>This message is automatically generated. </i></p>";
